export type Language = "en" | "bn";

export const translations = {
  en: {
    heroName: "Liza",
    heroSubtitle: "Happy Birthday, my love",

    section2Title: "My Only Desirable Love",
    section2Para1:
      "To the woman who completely redefined what it means to be alive. Before I met you, the world was ordinary. Now, every single moment is painted in the golden hues of your presence. You are the poetry I never knew how to write, the warmth I never knew I was missing.",
    section2Para2:
      "Finding your soulmate isn't just about finding someone you can live with; it's about finding the person you cannot imagine existing without. You are my heart's deepest, most profound certainty. Every smile, every touch, every quiet moment we share is a testament to how entirely I am yours.",

    section3Title: "The Inner Feelings for You My Heartbeat",
    section3Quote1: "\u201cYou are my home. The safest place my soul has ever known.\u201d",
    section3Quote2: "\u201cEvery day with you is a gift, unwrapped slowly in the quiet light of morning.\u201d",
    section3Quote3: "\u201cI love you, not only for what you are, but for what I am when I am with you.\u201d",

    section4Title: "My Heartful Thoughts for the Soul of My Life",
    section4Para1:
      "There are no words grand enough to hold what I feel for you. Language was invented long before you, and so it falls short every time I try. You are something the world had never seen before and I am the luckiest soul alive for getting to witness you every day.",
    section4Para2:
      "You carry within you a light that I have never seen in anyone else. It is not in how you look, though you are breathtaking. It is in how you love, how you laugh, how you face the world with a quiet, unshakeable grace that humbles me to my core.",
    section4Para3:
      "On this birthday, I want you to know that my life did not truly begin until you walked into it. Every year you are alive is a year the world is richer, warmer, and more beautiful. You are not just the love of my life, Liza. You are the reason I believe in something greater than myself.",
    section4Para4:
      "I hope this year holds everything your heart has ever longed for. And I hope you know, always and without doubt, that you are endlessly, completely, irrevocably loved by me.",
    section4Poem1:
      "I love you neither with my heart nor with my mind. Just in case the heart might stop, the mind can forget. I love you with my soul. Soul never stops or forgets.",
    section4Poem2:
      "I love you neither with my heart nor with my mind. Just in case the heart might stop, the mind can forget. I love you with my soul. Soul never stops or forgets.",
    section4Poem3:
      "If I met you today for the first time, I would fall for you all over again. But I wouldn\u2019t say a hello to you. I would just keep walking by \u2014 because I have already seen the ending for this movie, and I know I couldn\u2019t survive it.",

    section5Title: "Make a Wish",
    section5Hint: "Tap to blow out the candle",

    footerSign: "With all my love,",
    footerDate: "June 28, 2026",
  },

  bn: {
    heroName: "লিজা",
    heroSubtitle: "জন্মদিনের শুভেচ্ছা, আমার ভালোবাসা",

    section2Title: "আমার একমাত্র কাঙ্ক্ষিত ভালোবাসা",
    section2Para1:
      "সেই নারীকে, যে সম্পূর্ণভাবে পরিবর্তন করে দিয়েছে বেঁচে থাকার মানে। তোমার সাথে দেখা হওয়ার আগে, পৃথিবীটা ছিল সাধারণ। এখন প্রতিটি মুহূর্ত তোমার উপস্থিতির সোনালি আভায় রঙিন। তুমি সেই কবিতা, যা লিখতে আমি কখনো জানতাম না, সেই উষ্ণতা, যার অভাব আমি কখনো বুঝিনি।",
    section2Para2:
      "সত্যিকারের সঙ্গী খোঁজার মানে শুধু এমন কাউকে পাওয়া নয় যার সাথে বাঁচা যায়; এর মানে হলো সেই মানুষটাকে খুঁজে পাওয়া, যাকে ছাড়া নিজেকে কল্পনাও করা যায় না। তুমি আমার হৃদয়ের গভীরতম, সবচেয়ে নিশ্চিত সত্য। তোমার প্রতিটি হাসি, প্রতিটি স্পর্শ, প্রতিটি নীরব মুহূর্ত প্রমাণ করে — আমি সম্পূর্ণরূপে তোমার।",

    section3Title: "তোমার জন্য আমার হৃদয়ের গভীর অনুভূতি",
    section3Quote1: "\u201cতুমি আমার ঘর। আমার আত্মা যে সবচেয়ে নিরাপদ আশ্রয় পেয়েছে।\u201d",
    section3Quote2: "\u201cতোমার সাথে প্রতিটি দিন একটি উপহার, ভোরের শান্ত আলোয় ধীরে ধীরে খোলা।\u201d",
    section3Quote3: "\u201cআমি তোমাকে ভালোবাসি, শুধু তুমি যা তার জন্য নয়, বরং তোমার সাথে থাকলে আমি যা হই তার জন্য।\u201d",

    section4Title: "আমার জীবনের আত্মার প্রতি আমার হৃদয়ের কথা",
    section4Para1:
      "তোমার প্রতি যা অনুভব করি, তা ধারণ করার মতো কোনো শব্দ নেই। ভাষার জন্ম হয়েছিল তোমার আগে, তাই প্রতিবার চেষ্টা করলেই সে ব্যর্থ হয়। তুমি এমন কিছু যা পৃথিবী আগে কখনো দেখেনি — আর আমি সেই সৌভাগ্যবান আত্মা, যে প্রতিদিন তোমাকে দেখতে পায়।",
    section4Para2:
      "তোমার ভেতরে এমন একটি আলো আছে, যা আর কারো মধ্যে দেখিনি। এটা শুধু তোমার রূপে নয়, যদিও তুমি অসাধারণ সুন্দর। এটা তোমার ভালোবাসার ধরনে, তোমার হাসিতে, তোমার নীরব দৃঢ়তায় — যা আমাকে প্রতিদিন নত করে দেয়।",
    section4Para3:
      "এই জন্মদিনে, আমি চাই তুমি জানো — তোমার আগে আমার জীবন সত্যিকার অর্থে শুরু হয়নি। তুমি যতদিন আছো, পৃথিবী ততটুকুই সমৃদ্ধ, উষ্ণ, সুন্দর। তুমি শুধু আমার জীবনের ভালোবাসা নও, লিজা। তুমি সেই কারণ, যে কারণে আমি নিজের চেয়ে বড় কিছুতে বিশ্বাস করি।",
    section4Para4:
      "আশা করি এই বছরটা তোমার হৃদয়ের সব স্বপ্ন পূরণ করবে। আর সবসময় জেনো — তুমি অনন্তকাল, সম্পূর্ণভাবে, অটলভাবে আমার ভালোবাসায় আবৃত।",
    section4Poem1:
      "আমি তোমাকে ভালোবাসি না আমার হৃদয় দিয়ে, না মন দিয়ে। কারণ হৃদয় থেমে যেতে পারে, মন ভুলে যেতে পারে। আমি তোমাকে ভালোবাসি আমার আত্মা দিয়ে। আত্মা কখনো থামে না, কখনো ভোলে না।",
    section4Poem2:
      "আমি তোমাকে ভালোবাসি না আমার হৃদয় দিয়ে, না মন দিয়ে। কারণ হৃদয় থেমে যেতে পারে, মন ভুলে যেতে পারে। আমি তোমাকে ভালোবাসি আমার আত্মা দিয়ে। আত্মা কখনো থামে না, কখনো ভোলে না।",
    section4Poem3:
      "যদি আজ প্রথমবার তোমার সাথে দেখা হতো, আমি আবারও তোমার প্রেমে পড়ে যেতাম। কিন্তু হ্যালো বলতাম না। শুধু হেঁটে যেতাম পাশ দিয়ে — কারণ আমি ইতিমধ্যে এই গল্পের শেষটা দেখে ফেলেছি, আর জানি এটা থেকে বেঁচে ফেরা আমার পক্ষে সম্ভব নয়।",

    section5Title: "একটি ইচ্ছা করো",
    section5Hint: "মোমবাতি নেভাতে ট্যাপ করো",

    footerSign: "সমস্ত ভালোবাসা সহ,",
    footerDate: "জুন ২৮, ২০২৬",
  },
} as const;

export type TranslationKeys = keyof typeof translations.en;
